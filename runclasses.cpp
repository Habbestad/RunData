...
/*
std::vector<RunSummary> Run::getLaps(int lapLength)
{
    int lapNumber {1};
    TrackVecIt it1 = m_tracks.begin();
    TrackVecIt it2 = m_tracks.begin();
    std::string originalID = m_id;
    RunSummary lap;
    std::vector<RunSummary> lapVec;

    while(m_tracks.back().m_distance >= it2->m_distance + lapLength)
    {
        it2 = this->findClosestTrackByDistance(lapNumber*lapLength);
        RunSummary lap = this->getSection(it1, it2, originalID + std::to_string(lapNumber),true).getRunSummary();
        //lap.m_runTime = it2->m_time - it1->m_time;
        //lap.m_runDist = it2->m_distance - it1->m_distance;
        //lap.m_id = originalID + " - Lap" + std::to_string(lapNumber);

        lapVec.push_back(lap);
        lapNumber += 1;
        it1 = it2;
    }

/*
    lap.m_runTime = m_tracks.back().m_time - it2->m_time;
    lap.m_runDist = m_tracks.back().m_distance - it2->m_distance;
    lap.m_id = originalID + " - Lap" + std::to_string(lapNumber);
    lapVec.push_back(lap); 
